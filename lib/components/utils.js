...
export const handlePlaylistSelection = async (token, id, playlistName, setSongs) => {
  const response = await getSongsFromPlaylist(token, id);
  if (isValidPlaylist(response)){
    const formattedSongs = formatSongs(response.tracks);
